...
import { FETCH_API, SEARCH_RESULT_API, YOUTUBE_SEARCH_API } from '../utils/constant';
  ...
  try {
    ...
    {Vinfo && Vinfo.items && Vinfo.items.length > 0 && (
        <div className='mt-2 mb-2'>
          ...
          <p className='ml-5 font-bold text-xl '>{Vinfo.items[0].snippet.localized.title}</p>
          
          <div className='flex flex-row mt-3'>
            <img className=" mx-2 h-10 w-10 rounded-full" src={Vinfo.items[0].snippet.thumbnails.default.url} alt="thumbnil" />
            <p className='font-bold ml-5 text-xl  bg-gray-300 px-2 py-1 rounded-full'>{Vinfo.items[0].snippet.channelTitle}</p>
            <p className='m-1 text-green-600 ml-4 font-bold ml-24'>{Vinfo.items[0].statistics.viewCount/1000} views</p>
                <p className='text-green-600 m-1 ml-8 font-bold'>{Vinfo.items[0].statistics.likeCount} Likes</p>
            <p className='bg-red-600 font-bold text-white tracking-widest p-1 rounded-full px-10 ml-[50px]'>subscribe</p>
            </div>
      ...
      <h2 className='underline font-bold mb-8 text-2xl'>Suggested For You</h2>
